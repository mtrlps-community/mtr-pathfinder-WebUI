    ...
    try:
        ...
        if time:
            ...
            try:
                ...
                return jsonify({'error': '时间格式错误，请使用HH:MM:SS格式'})
        
        # 判断station是否为数字ID
        station_input = station
        try:
            station_short_id = int(station)
            # 如果是数字，尝试转换为车站名称
            from mtr_pathfinder_lib.mtr_timetable import station_short_id_to_id
            with open(config['LOCAL_FILE_PATH_V4'], 'r', encoding='utf-8') as f:
                data_v4 = json.load(f)
            sta_id = station_short_id_to_id(data_v4, station_short_id)
            if sta_id:
                station = data_v4['stations'][sta_id]['name']
        ...
        if text_mode:
            # 调用文字时刻表函数
            from mtr_pathfinder_lib.mtr_timetable import main_text_timetable
            result = main_text_timetable(
                config['LOCAL_FILE_PATH_V4'],
                '',
                departure_time if departure_time else int(time.time()) % 86400,
                station
            )
            if result:
            ...
            if direction:
                # 按方向查询
                from mtr_pathfinder_lib.mtr_timetable import main_get_sta_directions, main_sta_timetable
                
                # 获取车站方向
                directions_result = main_get_sta_directions(
                    config['LOCAL_FILE_PATH_V4'],
                    station,
                    os.path.join('templates', 'directions_template.htm')
                )
                ...
                html, size, directions_table = directions_result
                
                # 检查方向是否有效
                try:
                    direction_int = int(direction)
                    if direction_int not in directions_table:
                        return jsonify({'error': '路线编号错误'})
                    direction = direction_int
                except (ValueError, TypeError):
                    return jsonify({'error': '路线编号格式错误'})
                
                # 获取该方向的路线
                route_names = directions_table[direction]
                
                # 调用时刻表函数
                timetable_result = main_sta_timetable(
                    config['LOCAL_FILE_PATH_V3'],
                    config['LOCAL_FILE_PATH_V4'],
                    os.path.join('templates', 'station_template.htm'),
                    '',
                ...
                if timetable_result:
                    return jsonify({'result': timetable_result[0]})
                else:
                    return jsonify({'error': '未找到该车站信息'})
            elif routes:
                # 按指定路线查询
                from mtr_pathfinder_lib.mtr_timetable import main_sta_timetable
                ...
                # 直接查询车站所有方向
                from mtr_pathfinder_lib.mtr_timetable import main_get_sta_directions
                ...
                if directions_result:
                    return jsonify({'result': directions_result[0]})
                else:
                    return jsonify({'error': '未找到该车站信息'})
    except Exception as e:
